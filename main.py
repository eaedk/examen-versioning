...
# Dataset Splitting
# Please specify
to_ignore_cols = ["ID", "Id", "id", target_col]  # ID
...
if len(cat_cols) > 0:
    df_imputed_stacked_cat = cat_imputer.fit_transform(
        df.append(df).append(df)[cat_cols]
    )
    cat_ = (
        OneHotEncoder(sparse=False, drop="first")
        .fit(df_imputed_stacked_cat)
        .categories_
    )
else:
    cat_ = "auto"
...
transformers.append(("numerical", num_pipe, num_cols)) if len(num_cols) > 0 else None
transformers.append(
    (
        "categorical",
        cat_pipe,
...
preprocessor = ColumnTransformer(transformers=transformers).set_output(
    transform="pandas"
)
...
end2end_pipeline = Pipeline(
    [("preprocessor", preprocessor), ("model", RandomForestClassifier(random_state=10))]
